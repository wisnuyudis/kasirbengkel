...
jQuery(document).ready(function () {
	...
	$('#btn-excel').click(function() {
		...
		url = base_url + 'laporan-penjualan-perinvoice/ajaxExportExcel?start_date=' + start_date + '&end_date=' + end_date;
		fetch(url)
		  .then(resp => resp.blob())
		  .then(blob => {
	...
	$('#btn-pdf').click(function() {
		...
		url = base_url + 'laporan-penjualan-perinvoice/ajaxExportPdf?start_date=' + start_date + '&end_date=' + end_date + '&ajax=true';
		fetch(url)
		  .then(resp => resp.blob())
		  .then(blob => {
				$this.prop('disabled', false);
	...
	},	function(start, end, label) 
	{
		...
		$.ajax({
			url: base_url + 'laporan-penjualan-perinvoice/ajaxGetResumePenjualan?start_date=' + start_date + '&end_date=' + end_date,
			dataType: 'JSON',
			success: function(data) {
				$('#total-nilai').html( format_ribuan(data.total_neto) );
				$('#total-qty').html( format_ribuan(data.total_qty));
				$('#total-untung-rugi').html( format_ribuan(data.total_untung_rugi));
				console.log(data)
			}
		})
							
		settings.ajax.url = base_url + 'laporan-penjualan-perinvoice/getDataDTPenjualanPerinvoice?start_date=' + start_date + '&end_date=' + end_date;
		dataTables.destroy();
