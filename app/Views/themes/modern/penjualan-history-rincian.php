		...
		<form method="post" action="" class="form-horizontal" enctype="multipart/form-data">
			<div>
				...
				<div class="form-group row mb-3">
					...
					<div class="col-sm-6">
						<?=options(['name' => 'id_gudang', 'id' => 'gudang', 'disabled' => 'disabled'], $gudang, set_value('id_gudang', @$penjualan['id_gudang']))?>
					</div>
				</div>
				<div class="form-group row mb-3">
					<label class="col-sm-3 col-md-2 col-lg-3 col-xl-2 col-form-label">Harga</label>
					<div class="col-sm-6">
						<?=options(['name' => 'id_jenis_harga', 'id' => 'jenis-harga',  'disabled' => 'disabled'], $jenis_harga, set_value('id_jenis_harga', @$jenis_harga_selected))?>
					</div>
				</div>
							...
							<tbody>';
								...
								foreach ($barang as $val) {
									...
									<tr class="barang"'. $display .'>
										...
										<td>
											<input type="text" size="4" class="form-control text-end harga-satuan" name="harga_satuan[]" disabled value="' . format_number((int) @$val['harga_satuan']) . '"/>
											<input type="hidden" name="harga_pokok[]" class="harga-pokok" disabled value="' . @$val['harga_pokok'] . '"/>
										</td>
										<td>
											<div class="input-group">
												<button type="button" class="input-group-text qty-min disabled">-</button>
												<input type="text" class="form-control text-end qty" disabled style="width:52px" name="qty[]" value="' . format_number(@$val['qty']) . '"/>
												<button type="button" class="input-group-text qty-plus disabled">+</button>
											</div>
											<input type="hidden" name="id_barang[]" class="id-barang" disabled value="' . @$val['id_barang'] . '"/>
										</td>
										<td>
											<div class="input-group">'
											. options(['name' => 'diskon_barang_jenis[]', 'class' => 'diskon-barang-jenis', 'style' => 'width:auto',  'disabled' => 'disabled'],['%' => '%', 'rp' => 'Rp'], @$val['diskon_jenis']) 
											. '
											<input type="text" size="4" class="form-control text-end diskon-barang" style="width:100px" name="diskon_barang_nilai[]" disabled value="'. format_number(@$val['diskon_nilai']) . '"/>
											</div>
										</td>
										<td>
								...
								echo '</tbody>
											...
											<tr>
												...
												<th><input name="sub_total" class="form-control text-end" id="subtotal" type="text" disabled value="' . format_number( set_value('sub_total', $sub_total) ) . '" readonly/></th>
												<th></th>
											</tr>
											<tr>
												...
												<td>
													<div class="input-group">'
														. options(['name' => 'diskon_total_jenis', 'id' => 'diskon-total-jenis',  'disabled' => 'disabled', 'style' => 'flex: 0 0 auto;width: 70px'],['%' => '%', 'rp' => 'Rp'], set_value('diskon_total_jenis', @$penjualan['diskon_jenis']) ) 
														. '<input name="diskon_total_nilai" id="diskon-total" class="form-control text-end" disabled value="' . set_value('diskon_total_nilai', @$penjualan['diskon_nilai']) . '" type="text"/>
													</div>
												</td>
												<td></td>
											</tr>
											<tr>
												...
												<td>
													<div class="input-group">'
														. options(['name' => 'penyesuaian_operator', 'id' => 'operator-penyesuaian',  'disabled' => 'disabled', 'style' => 'flex: 0 0 auto;width: 70px'],['-' => '-', '+' => '+'], set_value('penyesuaian_operator', $penyesuaian_operator)) 
														. '<input name="penyesuaian_nilai" class="form-control text-end" id="penyesuaian" disabled value="'  . set_value('penyesuaian_nilai', $penyesuaian_nilai) . '" type="text"/>
													</div>
												</td>
												<td></td>
											</tr>';
											...
											if ($pajak['status'] == 'aktif') {
												...
												<tr>
													...
													<td>
														<div class="input-group">
															<button type="button" class="input-group-text" id="pajak-min" disabled>-</button>
															<input inputmode="numeric" id="pajak-nilai" type="text" class="form-control number text-end number" style="width:80px" disabled name="pajak_nilai" value="' . @$penjualan['pajak_persen'] . '"/>
															<span class="input-group-text">%</span>
															<button type="button" class="input-group-text" id="pajak-plus" disabled>+</button>
														</div>
													</td>
													<td></td>
												</tr>';
											...
											echo '<tr>
												...
												<th><input name="neto" class="form-control text-end" id="total" type="text" disabled value="' . format_number(set_value('neto', @$penjualan['neto'])) . '" readonly/></th>
												<th></th>
											</tr>
										</tbody>
										<tfoot>
											...
											<tr>
												...
												<td>' . options(['name' => 'jenis_bayar',  'disabled' => 'disabled'], ['tunai' => 'Tunai', 'tempo' => 'Tempo' ], set_value('jenis_bayar', @$penjualan['jenis_bayar'])) . '</td>
												<td></td>
											</tr>
										...
										$using_pembayaran = 1;
										if (empty($pembayaran)) {
											$pembayaran[] = ['jml_bayar' => 0, 'tgl_bayar' => date('Y-m-d'), 'id_user_bayar' => '',  'disabled' => 'disabled'];
											$using_pembayaran = 0;
										}
										$no = 1;
										...
										echo '<tr class="row-bayar">
												...
												<td colspan="4">
													<div class="input-group" style="width:250px; float:right">
														...
														<input type="text" size="1" name="tgl_bayar[]" disabled class="form-control flatpickr text-end format-ribuan" value="'. format_tanggal(@$val['tgl_bayar'], 'dd-mm-yyyy') .'"/>
													</div>
												<td><input type="text" size="1" name="jml_bayar[]" disabled class="form-control text-end format-ribuan item-bayar" value="'. format_number(@$val['jml_bayar']) .'"/></td>
												<td></td>
												</tr>';
										...
										echo '
											<tr>
												...
												<td><input class="form-control text-end format-ribuan kurang-bayar' . $class . '" type="text" name="kurang_bayar" disabled value="' . format_number( (int) set_value('kurang_bayar', @$penjualan['kurang_bayar'])) . '" required="required" readonly/>
												</th>
												<th></th>
											</tr>
										</tfoot>
