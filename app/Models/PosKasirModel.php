...
class PosKasirModel extends \App\Models\BaseModel
{
	...
	public function getListDataBarang($id_gudang, $id_jenis_harga)
	{
		...
		$sql = 'SELECT *, 
		(SELECT harga FROM barang_harga where id_barang=barang.id_barang AND id_jenis_harga=' . $id_jenis_harga . ' group BY id_barang ) 
		AS harga,
		(SELECT harga FROM barang_harga where id_barang=barang.id_barang AND jenis="harga_pokok" group BY id_barang ) 
		AS harga_pokok,
		(SELECT harga FROM barang_harga where id_barang=barang.id_barang AND id_jenis_harga=1 group BY id_barang)
		AS hargasatu,
		(SELECT harga FROM barang_harga where id_barang=barang.id_barang AND id_jenis_harga=2 group BY id_barang)
		AS hargadua,
		(SELECT harga FROM barang_harga where id_barang=barang.id_barang AND id_jenis_harga=3 group BY id_barang)
		AS hargatiga,
		(SELECT harga FROM barang_harga where id_barang=barang.id_barang AND id_jenis_harga=4 group BY id_barang)
		AS hargaempat,
		(SELECT harga FROM barang_harga where id_barang=barang.id_barang AND id_jenis_harga=5 group BY id_barang)
		AS hargalima
		FROM barang ' . $where . ' ' . $order . ' LIMIT ' . $start . ', ' . $length;
		...
		if ($id_barang) {
			...
			$list_stok = $this->getListStokByIdBarang($id_barang, $id_gudang);
			$list_harga = $this->getListHargaBarang($id_barang, $id_gudang);
		}
	...
	private function getListStokByIdBarang($id_barang, $id_gudang)
	{
		...
		$sql = 'SELECT id_barang, id_gudang, SUM(saldo_stok) AS stok FROM (
					SELECT id_barang, id_gudang, adjusment_stok AS saldo_stok, "adjusment" AS jenis
					FROM barang_adjusment_stok
					WHERE id_barang IN (' . $list_id_barang . ') AND adjusment_stok>"0" AND id_gudang=' . $id_gudang . '
						UNION ALL
					SELECT id_barang, id_gudang, CAST(qty as SIGNED) * -1 AS saldo_stok, "penjualan" AS jenis
					FROM penjualan_detail LEFT JOIN penjualan USING(id_penjualan) 
					WHERE id_barang IN (' . $list_id_barang . ') AND id_gudang=' . $id_gudang . '
						UNION ALL
					SELECT id_barang, id_gudang, qty_retur AS saldo_stok, "penjualan_retur" AS jenis
					FROM penjualan_retur_detail LEFT JOIN penjualan_detail USING(id_penjualan_detail) LEFT JOIN penjualan USING(id_penjualan)
					WHERE id_barang IN (' . $list_id_barang . ') AND id_gudang=' . $id_gudang . '
					...
					FROM transfer_barang_detail
					LEFT JOIN transfer_barang USING (id_transfer_barang)
					WHERE id_gudang_asal IN (' . $id_gudang . ')
						UNION ALL
					SELECT id_barang, id_gudang_tujuan, qty_transfer AS saldo_stok, "transfer_masuk" AS jenis 
					FROM transfer_barang_detail
					LEFT JOIN transfer_barang USING (id_transfer_barang)
					WHERE id_gudang_tujuan IN (' . $id_gudang . ')
				) AS tabel
				GROUP BY id_barang, id_gudang';
		// echo $sql; die;
		$result = $this->db->query($sql)->getResultArray();
