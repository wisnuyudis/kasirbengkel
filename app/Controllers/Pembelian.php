...
class Pembelian extends \App\Controllers\BaseController
{
	...
	public function add()
	{
		...
		if (!empty($_POST['id_barang'])) {
			$pembelian_detail = [];
			$list_index = ['id_barang', 'keterangan', 'expired_date', 'harga_satuan', 'qty', 'diskon_jenis', 'diskon_nilai', 'harga_neto'];
			...
			foreach ($_POST['diskon_jenis'] as $index => $val) {
				$pembelian_detail[$index]['diskon_jenis'] = $val;
			}

			foreach ($_POST['diskon_nilai'] as $index => $val) {
				$pembelian_detail[$index]['diskon_nilai'] = $val;
			}
		...
		foreach ($query['data'] as $key => &$val) {
			...
			$val['ignore_search_action'] = btn_action([
				'edit' => ['url' => $this->config->baseURL . $this->currentModule['nama_module'] . '/edit?id=' . $val['id_pembelian']], 'delete' => [
					'url' => '', 'id' =>  $val['id_pembelian'], 'delete-title' => 'Hapus data faktur pembelian nomor : <strong>' . $val['no_invoice'] . '</strong> ?'
				]
			]);
	...
	public function ajaxdttabletempo()
	{
		// header('Content-Type: application/json');
		$request = \Config\Services::request();
		$list = $this->model->getajaxdttabletempo();
		$data = array();
		$no = $request->getPost("start");
		foreach ($list as $tempo) {
			$no++;
			$row = array();
			$row[] = $no;
			$row[] = format_tanggal($tempo->tgl_invoice);
			$row[] = $tempo->no_invoice;
			$row[] = $tempo->nama_supplier;
			$row[] = format_number($tempo->total);
			$row[] = format_number($tempo->total_bayar);
			$row[] = format_number($tempo->kurang_bayar);
			$data[] = $row;
		}
		$output = array(
			"draw" => $request->getPost("draw"),
			"recordsTotal" => $this->model->countallajaxdttabletempo(),
			"recordsFiltered" => $this->model->countallajaxdttabletempo(),
			"data" => $data,
		);
	...
	function gethutang()
	{
		$get = $this->model->gethutang();
		$data = array(
			'pembelian' => format_number($get['pembelian']),
			'bayar' => format_number($get['bayar']),
			'hutang' => format_number($get['hutang'])
		);
	...
	function tes()
	{
		$data = $this->model->countallajaxdttabletempo();
