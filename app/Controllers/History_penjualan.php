...
class History_penjualan extends \App\Controllers\BaseController
{
	...
	public function index()
	{
		...
		$this->view('penjualan-history.php', $data);
	}
	...
	public function rincian()
	{
		...
		$this->data['title'] = 'Rincian Penjualan';
		$detail_data = $this->detailData();
		$this->data = array_merge($this->data, $detail_data);
		...
		if (@$_GET['mobile'] == 'true') {
			...
			$this->view('penjualan-history-rincian.php', $this->data);
		}
	}
	...
	public function getDataDTPenjualan() {
		...
		if(empty($startDate) && empty($endDate)) {
			$num_data = $this->model->countAllDataPenjualanCust();
			$query = $this->model->getListDataPenjualanCust();
			$startDate = '';
			$endDate = '';
		} else {
			$num_data = $this->model->countAllDataPenjualanCustDate($startDate, $endDate);
			$query = $this->model->getListDataPenjualanCustDate($startDate, $endDate);
		}
		...
		foreach ($query['data'] as $key => &$val) 
		{
			...
			$val['ignore_action'] = '<div class="btn-action-group">' . 
				btn_link(['url' => base_url() . '/history_penjualan/detail?id=' . $val['id_customer'] .'&startDate='. $startDate . '&endDate='. $endDate,'label' => '', 'icon' => 'fa fa-info-circle', 'attr' => ['target' => '_blank', 'class' => 'btn btn-success btn-xs me-1', 'data-bs-toggle' => 'tooltip', 'data-bs-title' => 'Detail Data'] ]) . 
				'</div>';
			$no++;
		}
	...
	//penjualan detail
	public function getDataDTPenjualanDetail() {
		...
		$num_data = $this->model->countAllDataPenjualanDetail($id,$startDate,$endDate);
		$result['draw'] = $start = $this->request->getPost('draw') ?: 1;
		$result['recordsTotal'] = $num_data;
		
		$query = $this->model->getListDataPenjualanCustDetail($id,$startDate,$endDate);
		$result['recordsFiltered'] = $query['total_filtered'];
		...
		foreach ($query['data'] as $key => &$val) 
		{
			...
			$val['ignore_urut'] = $no;
			$val['ignore_action'] = '<div class="btn-action-group">' . 
				btn_link(['url' => base_url() . '/history_penjualan/rincian?id=' . $val['id_penjualan'],'label' => '', 'icon' => 'fa fa-info-circle', 'attr' => ['target' => '_blank', 'class' => 'btn btn-success btn-xs me-1', 'data-bs-toggle' => 'tooltip', 'data-bs-title' => 'Detail Data'] ]) . 
			'</div>';
